...
class RequestHandler(object):
    ...
    def __init__(self, app,fn):#接受app参数
        ...
        self._has_name_kw_arg = has_named_kw_arg(fn)
        self._hase_var_kw_arg = has_var_kw_arg(fn)
        self._has_request_arg = has_request_arg(fn)

    async def __call__(self,request):
        kw = None
        if self._has_name_kw_arg or self._hase_var_kw_arg:
        ...
        if kw is None:
            ...
            if not self.has_var_kw_arg and self._name_kw_args:#当函数参数没有关键字参数时，移去request除命名关键字参数所有的参数信息
                copy = dict()
